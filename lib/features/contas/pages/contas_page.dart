...
class ContasPage extends StatelessWidget {
  ...
  Future<void> _excluirConta(BuildContext context, dynamic conta) async {
    final confirmar = await showDialog<bool>(
      context: context,
      builder: (dialogContext) => AlertDialog(
        title: const Text('Excluir conta?'),
        content: Text(
          'Excluir "${conta.nome}"? A conta só poderá ser removida se não houver movimentações vinculadas.',
        ),
        actions: [
          TextButton(
    ...
    try {
      await repository.excluir(conta.id);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Não foi possível excluir esta conta porque ela possui movimentações vinculadas.',
          ),
        ),
      );
    }
